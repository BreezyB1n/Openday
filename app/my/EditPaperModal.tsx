...
type Props = {
  item: Paper
  onSave: (updated: Paper) => void
  onDelete: (id: string) => void
  onClose: () => void
}

export default function EditPaperModal({ item, onSave, onDelete, onClose }: Props) {
  const [form, setForm] = useState<Paper>({ ...item })

  function set(field: keyof Paper, value: string | number) {
  ...
  function handleDelete() {
    if (!window.confirm('删除这条论文记录？')) return
    onDelete(item.id)
    onClose()
  }
      ...
      <div className="w-[420px] bg-bg-primary border-l border-border flex flex-col overflow-hidden">
        ...
        <div className="bg-bg-inverse text-text-inverse px-5 py-3 flex justify-between items-center shrink-0">
          <div>
            ...
            <div className="text-[13px] tracking-tight">RESEARCH & PAPER</div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="作者顺序">
              <input value={form.authorOrder} onChange={e => set('authorOrder', e.target.value)}
                placeholder="第一作者"
                className="w-full bg-bg-secondary border border-border px-3 py-2 text-[12px] focus:outline-none focus:border-text-primary" />
            </Field>
            <Field label="发表年份">
              <input value={String(form.year)} onChange={e => set('year', parseInt(e.target.value) || form.year)}
                placeholder="2025"
                className="w-full bg-bg-secondary border border-border px-3 py-2 text-[12px] focus:outline-none focus:border-text-primary" />
            </Field>
          </div>
